...
def get_wallet_balance(substrate, wallet):
...
def WalletMon():
    ...
    # Configurations
    discord_url = config.get('DISCORD_WALLET_NOTIFICATIONS', None)

    wallet = config.get('WALLET', None)
    ...
    if wallet:
        ...
        while True:
            try:
                current_balance = get_wallet_balance(substrate, wallet)
